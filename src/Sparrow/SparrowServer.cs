...
namespace Sparrow {
	public class SparrowServer {
		public SparrowServer (Assembly assembly, string jwt_secret) {
			m_assembly = assembly;
		...
		public void set_res_from_path (string _path = "D:/wwwroot") {
			_path = _path.Replace ('\\', '/');
			m_res_path = (_path.right_is ("/") || _path.is_null () ? _path : $"{_path}/");
			m_res_namespace = "";
		}

		public void set_res_from_namespace (string _namespace = "TestServer.res") {
			m_res_path = "";
			m_res_namespace = (_namespace.right_is (".") || _namespace.is_null () ? _namespace : $"{_namespace}.");
		}

		public void set_log_path (string _log_path = "D:/sparrow/log") {
			_log_path = _log_path.Replace ('\\', '/');
			Log.m_path = (_log_path.right_is ("/") || _log_path.is_null () ? _log_path : $"{_log_path}/");
		}

		public void set_ssl_file (string _pfx_file, string _pwd = "") {
			m_pfx = _pfx_file.is_null () ? null : (_pwd.is_null () ? new X509Certificate2 (_pfx_file) : new X509Certificate2 (_pfx_file, _pwd));
		}
		...
		public void run (ushort port) {
			try {
				...
				foreach (var _module in m_assembly.GetTypes ()) {
					string _module_name = _module.Name;
					if (_module_name [0] == '_') {
						var (_num, _tmp2) = _module_name.mid ("_").split2 ('_');
						if (int.TryParse (_num, out int _r))
							_module_name = _tmp2;
					}
					...
					if (_http_module_attr != null) {
						...
						_builder?.add_module (_module_name, _module_prefix, _http_module_attr.m_description);
						//
						...
						Action<MethodInfo> _process_method = (_method) => {
							...
							if (_jwt_type == "Connect") {
								...
								foreach (var _param in _params) {
									...
									if (!_method_attr.HideDoc)
										_builder?.add_param (_module_name, _method_attr.Type, _method.Name, _param.Name, _param.ParameterType.Name, _param_desp);
								}
								...
								m_api_handlers.Add (_path_prefix, new RequestStruct ($"{_module_name}.{_method.Name}", _method, _jwt_reconnect_func, _jwt_type));
							}
						};
		...
		private X509Certificate2 m_pfx = null;
